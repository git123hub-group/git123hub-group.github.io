...
var op=function(){tx=db.transaction(random_like_name,"readwrite");store=tx.objectStore(random_like_name);var a=store.get("History");a.onsuccess=function(){rez=a.result;var c=rez.data;historyA[0]=+new Date;historyS+=(new Date(historyA[0])).toString()+"\n";for(var b=1;50>=b;++b){historyA[b]=c[(rez.ptr+50-b)%50];if(null==historyA[b])break;historyS+=(new Date(historyA[b])).toString()+"\n"}c[rez.ptr]=historyA[0];rez.ptr=(rez.ptr+1)%50;document.getElementById("historyB").innerHTML=historyS;db.transaction(random_like_name,
